...
page_stat_schema = types.StructType([
...
def main(in_directory, out_directory):
    ...
    stats = spark.read.csv(in_directory, schema=page_stat_schema, sep=' ').withColumn('filename', path_to_hour(functions.input_file_name()))
    stats = stats.filter((stats['lan'] == 'en') & (stats['name'] != 'Main_Page') & ~(stats['name'].startswith('Special:')))
